...
public:
    ...
    MapHash(): num_elems(0), capacity(INITIAL_CAPACITY), buckets(new List[INITIAL_CAPACITY]) { };
    MapHash(const MapHash &other): num_elems(other.num_elems), hash(other.hash), capacity(other.capacity), buckets(new List[other.capacity]) {
...
private:
    ...
    static typename List::const_iterator find_in_list(const List &list, const K &key) {
        auto it = list.begin();
