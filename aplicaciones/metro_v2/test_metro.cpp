...
TEST_CASE("Tiempo de espera") {
  ...
  SECTION("Tiempos en línea inexistente") {
    REQUIRE_THROWS_WITH(m.tiempo_proximo_tren("2", "Falsa", {0,0,0}), "línea no encontrada");
  }

  SECTION("Tiempos en parada inexistente") {
    REQUIRE_THROWS_WITH(m.tiempo_proximo_tren("1", "Falsa", {0,0,0}), "parada no encontrada");
  }
