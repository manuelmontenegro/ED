...
TEST_CASE("Añadir estudiantes y cursos", "Academia") {
  ...
  SECTION("Añadir un estudiante que ya existe debe fallar") {
    REQUIRE_THROWS_WITH(ac.anyadir_estudiante("1", "Nadie", "Nadie"), "estudiante ya existente");
  }

  SECTION("Obtener estudiantes en un curso que no existe") {
    REQUIRE_THROWS_WITH(ac.estudiantes_matriculados("Curso 0"), "curso no encontrado");
  }
  ...
  SECTION("Obtener estudiantes y plazas en un curso que existe sin estudiantes") {
  ...
  SECTION("Matricular estudiantes en varios cursos") {
    ...
    for (const std::string &c : cursos) {
      ...
      REQUIRE(std::binary_search(matriculados.begin(), matriculados.end(), "Ramirez, Bertoldo"));
    }

    auto cursos_est = ac.cursos_estudiante("1");
    REQUIRE(std::is_permutation(cursos_est.begin(), cursos_est.end(), cursos.begin()));
  }
  ...
  SECTION("Matricular estudiantes en varios cursos y dar de baja") {
    ...
    auto cursos_est = ac.cursos_estudiante("1");
    REQUIRE(find(cursos_est.begin(), cursos_est.end(), "Curso 1") == cursos_est.end());
    REQUIRE(find(cursos_est.begin(), cursos_est.end(), "Curso 2") != cursos_est.end());
    REQUIRE(find(cursos_est.begin(), cursos_est.end(), "Curso 3") != cursos_est.end());
  }
  ...
  SECTION("Matricularse en curso lleno") {
    ...
    auto matriculados = ac.estudiantes_matriculados("Curso 1");
    REQUIRE(find(matriculados.begin(), matriculados.end(), "3") == matriculados.end());
    ...
    REQUIRE(find(matriculados.begin(), matriculados.end(), "Ramirez, Bertoldo") != matriculados.end());
    REQUIRE(find(matriculados.begin(), matriculados.end(), "Perez, Bertolda") == matriculados.end());
    REQUIRE(find(matriculados.begin(), matriculados.end(), "Distrajez, Modicio") != matriculados.end());
  }
}