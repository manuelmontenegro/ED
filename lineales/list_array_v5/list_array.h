...
template<typename T>
void ListArray<T>::push_front(const T &elem) {
...
template<typename T>
void ListArray<T>::push_back(const T &elem) {
  if (num_elems == capacity) {
    resize_array(capacity * 2);
  }
...
template<typename T>
void ListArray<T>::resize_array(int new_capacity) {
...
template<typename T>
void ListArray<T>::display(std::ostream &out) const {
  out << "[";
  if (num_elems > 0) {
    out << elems[0];
    for (int i = 1; i < num_elems; i++) {
