...
TEST_CASE("creation of sets", "SetList") {
  SetList<int> s;
  ...
  SECTION(
      "if two elements are inserted in reverse order, its cardinality is 2") {
    s.insert(2);
    s.insert(4);
    REQUIRE(s.size() == 2);
  }
  ...
  SECTION(
      "if two elements are inserted in reverse order, both are in the set") {
    s.insert(4);
    s.insert(2);
    REQUIRE(s.contains(2));
    REQUIRE(s.contains(4));
...
TEST_CASE("removing elements from sets", "SetList") {
  SetList<int> s;
...
TEST_CASE("set larger than capacity", "SetList") {
  SetList<int> s;
  for (int i = 0; i < 1000; i += 10) {
    s.insert(i);
  }

  SECTION("count elements in the set") { REQUIRE(s.size() == 100); }

  SECTION("multiples of 10 are in the set") {
