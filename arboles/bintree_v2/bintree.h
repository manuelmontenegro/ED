...
public:
  ...
  BinTree(const BinTree &left, const T &elem, const BinTree &right)
    :root_node(new TreeNode { copy_with_children(left.root_node), elem, copy_with_children(right.root_node) }) { }
...
private:
  ...
  static TreeNode * copy_with_children(const TreeNode *root) {
    if (root == nullptr) {
      ...
      return new TreeNode { copy_with_children(root->left), root->elem, copy_with_children(root->right) };
    }
  }
